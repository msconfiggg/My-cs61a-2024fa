...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def about_subject(paragraph):
        paragraph = remove_punctuation(paragraph)
        paragraph = lower(paragraph)
        new_paragraph = split(paragraph)
        for i in range(len(subject)):
            for j in range(len(new_paragraph)):
                if subject[i] == new_paragraph[j]:
                    return True
        return False
...
def accuracy(typed, source):
    ...
    "*** YOUR CODE HERE ***"
    match_count = 0
    min_num = min(len(typed_words), len(source_words))
...
def furry_fixes(typed, source, limit):
    ...
    result = 1 + furry_fixes(typed[1:], source[1:], limit - 1)
    return result
    # END PROBLEM 6
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(len(timestamps_per_player[i]) - 1):
            player_time.append(timestamps_per_player[i][j + 1] - timestamps_per_player[i][j])
...
def fastest_words(match_object):
    ...
    for i in word_indices:
        fastest_player = min(player_indices, key=lambda j: get_time(match_object, j, i))
        fastest[fastest_player].append(get_word(match_object, i))
    return fastest
    # END PROBLEM 11
