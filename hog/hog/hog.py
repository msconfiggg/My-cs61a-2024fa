...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    ones_number = player_score % 10
    tens_number = opponent_score // 10 % 10
    return max(abs((ones_number - tens_number)*3), 1)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    initial_one = strategy(0, 0)
    for i in range(goal):
        for j in range(goal):
            if strategy(i, j) != initial_one:
                return False
    return True
    # END PROBLEM 7
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    result = sus_update(0, score, opponent_score) - score
    if result >= threshold:
        return 0
    else:
        return num_rolls
